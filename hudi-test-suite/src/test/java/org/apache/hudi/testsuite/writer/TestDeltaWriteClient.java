...
import org.apache.hudi.testsuite.HoodieDeltaStreamerWrapper;
...
@PrepareForTest({DeltaWriteClient.class})
public class TestDeltaWriteClient {
  ...
  public void testWorkloadWriterWithDeltaStreamer() throws Exception {
    ...
    DeltaWriteClient deltaWriteClient = new DeltaWriteClient(mockSC, props, mockCfg, schema);
    assertEquals(deltaWriteClient.fetchSource().getRight().getKey(), checkPoint);
    assertEquals(deltaWriteClient.fetchSource().getRight().getValue(), null);
    assertFalse(deltaWriteClient.startCommit().get() == commitTime);
    assertTrue(deltaWriteClient.bulkInsert(Option.of(commitTime)) == null);
    assertTrue(deltaWriteClient.insert(Option.of(commitTime)) == null);
    assertTrue(deltaWriteClient.upsert(Option.of(commitTime)) == null);
  }
  ...
  public void testWorkloadWriterWithHoodieWriteClient() throws Exception {
    ...
    assertEquals(deltaWriteClient.fetchSource().getRight().getKey(), checkPoint);
    assertEquals(deltaWriteClient.fetchSource().getRight().getValue(), null);
    assertEquals(deltaWriteClient.startCommit().get(), commitTime);
    assertTrue(deltaWriteClient.bulkInsert(Option.of(commitTime)) == null);
    assertTrue(deltaWriteClient.insert(Option.of(commitTime)) == null);
    assertTrue(deltaWriteClient.upsert(Option.of(commitTime)) == null);
  }
